...
// BAKER IDEA:
// grr...for AP->APML conversion why not do something like how I did APML->HTML?
// eg. {a: "[\{]", b: "[["}
...
function isBlockStatement(line){
  ...
  for(var i=0; i<multilineBlocks.length; i++){
    if(line.indexOf(multilineBlocks[i]) >= 0){
      debug("found block statement "+line);
      return true;
    }
  }
  return false;
...
function apml2ap(code){
  ...
  for(var i=0; i<lines.length; i++){
    ...
    if(line.search(/^[A-Za-z0-9_]{1}[ ]*$/g)>=0){
      ...
      line = line.replace(/[ ]*$/g, "");
      line = line+" ()";
      debug("\tFOUND procedure call line? "+line);
    }
...
function pipe2parens(line){
  ...
  debug("AFTER open paren procedure: "+line);

  // now, remaining pipes are all closing parens
  line = line.replace(/[\|]/g,")");

  debug("AFTER close paren procedure: "+line);
...
function processProcedureCalls(line, tabs){
  ...
  debug(tabs+"for >>"+line+"<< innermost starts at: "+innerMost);
...
function findNoParamProcedures(line){
  ...
  // ASSUMPTION: processProcedureCalls has been run first.  So foo("a") --> [foo |a|]  but foo() --> [foo]
...
$( document ).ready(function() {
    ...
    debug( "ready! adding event listeners" );
